...
TsIRTContinuous::TsIRTContinuous(TsParameterManager* pM, G4String parmName): TsVIRTProcedure(pM,parmName){
	...
	fNx = round((fXMax-fXMin)/fBinWidth) == 0 ? 1 : round((fXMax-fXMin)/fBinWidth);
...
TsIRTContinuous::~TsIRTContinuous() {;}


void TsIRTContinuous::runIRT(G4double, G4double, G4double, G4bool) {
	ConductReactions();
	CleanIRTVariables();
}


void TsIRTContinuous::Sampling(G4int pulse){
	if ( !fScorersInitialized )
		initializeScorers();

	if( 0 < fVerbosity ) G4cout<<pulse<<" th pulse start at "<<fPulseTime[pulse]/ps<<" ps with "<<fPulseMoleculeMap[pulse].size()<<" molecules"<<G4endl;

	for(auto it = fPulseMoleculeMap[pulse].begin(); it != fPulseMoleculeMap[pulse].end(); ++it){
		TsIRTConfiguration::TsMolecule aMol = (*it);
...
//		}
		...
		G4int K = fUtils->FindBin(fNz, fZMin, fZMax, fChemicalSpecies[fSpeciesIndex].position.z());
		fSpaceBinned[I][J][K][fSpeciesIndex] = true;

		sampleReactions(fSpeciesIndex);
...
void TsIRTContinuous::AddMolecule(TsIRTConfiguration::TsMolecule aMol, G4int pulse) {
	if ( !fScorersInitialized )
		initializeScorers();

	if(fStartPulse != aMol.trackID){
		fPulseTime.push_back(aMol.time);
		fStartPulse = aMol.trackID;
	}

	fPulseMoleculeMap[pulse].push_back(aMol);
}


void TsIRTContinuous::AddMolecule(TsIRTConfiguration::TsMolecule aMol) {
    G4ThreeVector position = aMol.position;
...
void TsIRTContinuous::AddMolecule(G4Track* aTrack, G4double time, G4int moleculeID, G4ThreeVector offset){
...
void TsIRTContinuous::Homogenize(){

	G4int size = (G4int) fChemicalSpecies.size();

	if(fVerbosity > 0)	G4cout<<"End of pulse: "<<fEndPulse<<'\t'<<fEndTime/ps<<" ps"<<G4endl;

	auto it = fChemicalSpecies.begin();
	while(it != fChemicalSpecies.end()){
		...
		if(it->second.trackID == fEndPulse){
			fConcentrations[it->second.id]++;
			++it;
			RemoveMolecule(index);
		}else{
			++it;
	...
	for ( auto& nameAndID : fMolecules ) {
		...
		fGValues[name][fEndPulse] += fConcentrations[id];
	}
	fConcentrations.clear();
}


void TsIRTContinuous::sampleReactions(G4int i) {
	...
	FindBinIndexes(thisPos, fBinWidth);
			...
			for ( int kk = fziniIndex; kk <= fzendIndex; kk++ ) {
				for (auto& IndexAndAlive:fSpaceBinned[ii][jj][kk]) {
					...
					if( -1 < indexOfReaction ) {
						...
						if ( (0 < irt) && (irt + gTime <= fTimeCut) && (fMinTime <= irt + gTime) && (irt < fNHTimeCut)) {
							irt += gTime;
...
void TsIRTContinuous::ConductReactions() {
	G4int pulses = fPulseTime.size();

	G4cout<<"Total "<<pulses<<" pulses will be proceed"<<G4endl;
	Sampling(0);

	fStartPulse = 1;
	fEndPulse = 0;

	fStartTime = fPulseTime.size() > 1 ? fPulseTime.at(fStartPulse) : DBL_MAX;
	fEndTime = fPulseTime[fEndPulse] + fNHTimeCut;

	G4int tmp_trackID = 0;
	G4double irt = 0;

	while ((pulses > fEndPulse) || (irt < fTimeCut )) {

		G4int currentIRT = 0;
		if(fIRTValues.empty()){
			irt = DBL_MAX;
		}else{
			irt = fIRTValues[currentIRT].first;
		}


		if(irt > fStartTime || irt > fEndTime){
			if(fStartTime < fEndTime && fPulseTime[fStartPulse] > 0){
				Sampling(fStartPulse);
				fStartPulse++;
				fStartTime = fStartPulse < pulses ? fPulseTime[fStartPulse] : DBL_MAX;
				continue;
			}else{
				Homogenize();
				fEndPulse++;
				fEndTime = fEndPulse < pulses ? fPulseTime[fEndPulse] + fNHTimeCut : DBL_MAX;
				continue;
			}
		}
		...
		if ( irt < fTimeCut ) {
			...
			if ( !fIRTIsBackground[idx] ) {
				...
				tmp_trackID = std::max(fChemicalSpecies[iM].trackID, fChemicalSpecies[jM].trackID);
...
//				fConcentrations[fChemicalSpecies[jM].id]--;
				...
				tmp_trackID = fChemicalSpecies[iM].trackID;
...
//						}
				...
				if(fChemVerbosity > 0){
					...
					G4cout<<"At rest: "<<irt-fMinTime<<" ns"<<'\t'<<fMoleculesName[fChemicalSpecies[iM].id]<<" ("<< iM << ") (" << fChemicalSpecies[iM].id<<") "
								<<" + "<<fMoleculesName[molB]<<" ("<<molB<<") => ";
				}
...
std::vector<TsIRTConfiguration::TsMolecule> TsIRTContinuous::GetSurvivingMoleculesWithMolID(G4int molID) {
...
void TsIRTContinuous::Clean() {
	...
	fEndPulse = -1;
	fPulseTime.clear();
	fPulseMoleculeMap.clear();
}

void TsIRTContinuous::CleanIRTVariables() {
	fIRTIndex.clear();
	fIRTValues.clear();
	fIRTMolA.clear();
	fIRTMolB.clear();
...
G4bool TsIRTContinuous::MoleculeExists(G4int Index) {
...
void TsIRTContinuous::RemoveMolecule(G4int Index) {
...
void TsIRTContinuous::SetContainersForNextPulse() {;}
